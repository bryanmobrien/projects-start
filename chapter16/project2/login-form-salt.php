...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $pass === '') {
        ...
        if ($user && !empty($user['salt']) && !empty($user['password_sha256'])) {
            $digest = hash('sha256', $pass . $user['salt']);

            if (hash_equals($user['password_sha256'], $digest)) {
...
<section class="flex flex-col w-full max-w-md px-4 py-8 bg-white rounded-lg shadow dark:bg-gray-800 sm:px-6 md:px-8 lg:px-10">
    ...
    <div class="flex items-center justify-center mt-6">
        <a href="registration-form-salt.php" class="text-xs text-gray-500 hover:text-gray-700 dark:text-gray-100">
            Register using salt/SHA-256.
        </a>
    </div>

    <div class="flex items-center justify-center mt-2">
        <a href="login-form-bcrypt.php" class="text-xs text-gray-500 hover:text-gray-700 dark:text-gray-100">
            Try bcrypt login.
        </a>
    </div>
